...
| `add_reserved_member.py` | **DG01 only.** Adds a `FILE_TYPE_FW_RESERVED_ZONE_FILE` (type 0x32) member to a `.ufw`, which makes the RCSP loader erase the matching reserved area and download the member into it — the supported way to restore a reserved region over the air. The name must go in `szReserveZoneName` (entry byte 36), not the trailing `name[16]`, and the payload is scrambled with the chipkey anchored to its offset **in the container**. Proves its own output by re-applying the cipher and asserting the result equals the plaintext. | `python3 scripts/add_reserved_member.py <in.ufw> <AREA_NAME> <payload.bin> -o <out.ufw> [--chipkey 0xB165]` |
| `fix_area_table.py` | **DG01 only.** Grafts the OEM's `EXT_RESERVED`/`RESFS` directory into a built `.ufw`'s app-area chain. `isd_download` cannot emit it: it forces one globally ascending address order and sorts `[RESERVED_EXPAND_CONFIG]` last, so `RESFS` (`0x1F7000`) after `BTIF` (`0x3FE000`) is rejected. The two 32-byte entries are copied verbatim from the OEM dump (their CRCs are position-independent); only the container's offsets, `imgsize` and CRCs are rebuilt. Idempotent. | `python3 scripts/fix_area_table.py <in.ufw> [-o <out.ufw>] [--oem firmware/oem_DG01.bin] [--chipkey 0xB165]` |
| `check_dg01_geometry.py` | **DG01 only.** Asserts a packed image declares the unit's factory table (`PRCT`/`VM`/`MODE`/`EXIF`/`BTIF`/`RESFS` and `app_dir_head == 0x1000`). A mismatch makes the update loader write nothing and still report success, so this runs on every `--board dg01 --ota` build — it is the guard against the code-size cliff at `CODE_LEN 0xF7000`. It also asserts `RESFS` keeps `attr=0x92`/`res=0x81` (OPT 1, "do not operate") and refuses a `.ufw` carrying a member named `RESFS` — in a firmware build that would be an accident and a 2 MB erase; deliberate restore images pass `--allow-resfs-member`. | `python3 scripts/check_dg01_geometry.py <flash.bin\|*.ufw> [chipkey_hex] [--allow-resfs-member]` |
